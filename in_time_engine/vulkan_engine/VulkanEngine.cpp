...
namespace it {
	...
	// Properties
	Vulkan::VulkanEngine* Vulkan::VulkanEngine::s_singleton = nullptr;
	std::vector<const char*> Vulkan::VulkanEngine::s_validationLayersToEnable{ "VK_LAYER_KHRONOS_validation" };
	std::vector<std::string> Vulkan::VulkanEngine::s_availableValidationLayers = std::vector<std::string>(0);
	std::vector<const char*> Vulkan::VulkanEngine::s_extensionsToEnable = { VK_KHR_SWAPCHAIN_EXTENSION_NAME };

	// Functions
	std::vector<const char*> Vulkan::VulkanEngine::s_getGLFWRequiredExtensions() {
	...
	// Properties
#if defined(_DEBUG)
	bool Vulkan::VulkanEngine::s_enableValidationLayers = true;
#endif
#if defined(NDEBUG)
	bool Vulkan::VulkanEngine::s_enableValidationLayers = false;
#endif

	// Getters
	Vulkan::VulkanEngine* Vulkan::VulkanEngine::s_getSingleton() {
		return s_singleton;
	}
	std::vector<const char*> Vulkan::VulkanEngine::s_getValidationLayersToEnable() {
		return s_validationLayersToEnable;
	}
	std::vector<std::string> Vulkan::VulkanEngine::s_getAvailableValidationLayers() {
		return s_availableValidationLayers;
	}

	void Vulkan::VulkanEngine::s_init() {
	...
	// Functions
	void Vulkan::VulkanEngine::createVulkanInstance() {
	...
	void Vulkan::VulkanEngine::initDebugMessenger() {
		if (!s_enableValidationLayers)
			return;
	...
	void Vulkan::VulkanEngine::pickPhysicalDevice() {
	...
	bool Vulkan::VulkanEngine::isPhysicalDeviceSuitable(VkPhysicalDevice physicalDevice) {
	...
	Vulkan::VulkanEngine::QueueFamilyIndices Vulkan::VulkanEngine::findQueueFamilies(VkPhysicalDevice physicalDevice) {
	...
	void Vulkan::VulkanEngine::createLogicalDevice() {
	...
	void Vulkan::VulkanEngine::createSurface() {
		if (glfwCreateWindowSurface(instance, glfwWindow, nullptr, &surface) != VK_SUCCESS) {
			throw std::runtime_error("failed to create window surface!");
		}
	}
	bool Vulkan::VulkanEngine::checkDeviceExtensionSupport(VkPhysicalDevice physicalDevice) {
	...
	Vulkan::VulkanEngine::SwapChainSupportDetails Vulkan::VulkanEngine::querySwapChainSupport(VkPhysicalDevice physicalDevice) {
	...
	VkSurfaceFormatKHR Vulkan::VulkanEngine::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
	...
	VkPresentModeKHR Vulkan::VulkanEngine::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
	...
	VkExtent2D Vulkan::VulkanEngine::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
	...
	void Vulkan::VulkanEngine::createSwapChain() {
	...
	void Vulkan::VulkanEngine::createImageViews() {
	...
	void Vulkan::VulkanEngine::createRenderPass() {
	...
	void Vulkan::VulkanEngine::createGraphicsPipeline() {
		// Vertex shader module
		Vulkan::ShaderModule vertexShaderModule = Vulkan::ShaderModule();
		...
		Vulkan::ShaderModule fragmentShaderModule = Vulkan::ShaderModule();
	...
	void Vulkan::VulkanEngine::createFrameBuffers() {
	...
	void Vulkan::VulkanEngine::createCommandPool() {
	...
	void Vulkan::VulkanEngine::createCommandBuffer() {
	...
	void Vulkan::VulkanEngine::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
	...
	void Vulkan::VulkanEngine::createSyncObjects() {
	...
	void Vulkan::VulkanEngine::cleanupSwapChain() {
	...
	void Vulkan::VulkanEngine::recreateSwapChain() {
	...
	void Vulkan::VulkanEngine::createVertexBuffer() {
		// Vertex buffer
		...
		vertexBuffer = new Vulkan::Buffer(vertexBufferCreateInfo, (void*)vertices.data(), 0ULL, vertices.size(), VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
	}

	// Object | public

	// Constructor / Destructor
	Vulkan::VulkanEngine::VulkanEngine(GLFWwindow* glfwWindow) : glfwWindow(glfwWindow) {
	...
	Vulkan::VulkanEngine::~VulkanEngine() {
		vkDeviceWaitIdle(vulkanDevice);
	...
	// Functions
	void Vulkan::VulkanEngine::render() {
		if (!shouldRender)
			return;
	...
	void Vulkan::VulkanEngine::onSurfaceResize(int width, int height) {
