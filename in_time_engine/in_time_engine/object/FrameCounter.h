...
namespace it {
	class FrameCounter : public UpdateEvent, public PhysicsUpdateEvent2D {
		...
		private:
			...
			std::chrono::nanoseconds nextPhysicsUpdateTime = InTimeEngine::s_getSingleton()->fixedTime.getTime() + std::chrono::seconds(1);
		...
		public:
			...
			void onPrePhysicsUpdate2D() {
				...
				if (fixedTime.getTime() >= nextPhysicsUpdateTime) {
