...
util::IdFactory::IdFactory() {
	idRangeList.push_back(std::pair<int, int>(0, 0));
}
util::IdFactory::IdFactory(int idCount) {
	if (idCount <= 0)
		idCount = 1;
...
// Getters
std::list<std::pair<int, int>> util::IdFactory::getIdRangeList() const {
	return idRangeList;
}
int util::IdFactory::getRegisteredIdsCount() const {
	return registeredIdsCount;
}
int util::IdFactory::getIdsAvailableCount() const {
	return idsAvailableCount;
}


// Functions
bool util::IdFactory::resize(int idAmount) {
	// Error check
	if (idAmount <= 0)
		return false;
...
bool util::IdFactory::has(int id) const {
	for (const std::pair<int, int>& idRange : idRangeList)
		if (idRange.first <= id && id <= idRange.second)
			return true;
	return false;
}
bool util::IdFactory::pop(int id) {
	if (idRangeList.size() == 0)
		resize(registeredIdsCount * 2);
...
bool util::IdFactory::push(int id) {
	// Error check
