...
static bool ImGui_ImplGlfw_ShouldChainCallback(GLFWwindow* window)
{
    ...
    return bd->CallbacksChainForAllWindows ? true : (window == bd->WindowWrapper);
}
...
void ImGui_ImplGlfw_InstallCallbacks(GLFWwindow* window)
{
    ...
    IM_ASSERT(bd->WindowWrapper == window);
...
static bool ImGui_ImplGlfw_Init(GLFWwindow* window, bool install_callbacks, GlfwClientApi client_api)
{
    ...
    bd->WindowWrapper = window;
    bd->Time = 0.0;

    ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
#if GLFW_VERSION_COMBINED < 3300
    platform_io.Platform_SetClipboardTextFn = [](ImGuiContext*, const char* text) { glfwSetClipboardString(ImGui_ImplGlfw_GetBackendData()->WindowWrapper, text); };
    platform_io.Platform_GetClipboardTextFn = [](ImGuiContext*) { return glfwGetClipboardString(ImGui_ImplGlfw_GetBackendData()->WindowWrapper); };
#else
    ...
    main_viewport->PlatformHandle = (void*)bd->WindowWrapper;
#ifdef _WIN32
    main_viewport->PlatformHandleRaw = glfwGetWin32Window(bd->WindowWrapper);
#elif defined(__APPLE__)
    main_viewport->PlatformHandleRaw = (void*)glfwGetCocoaWindow(bd->WindowWrapper);
#else
...
void ImGui_ImplGlfw_Shutdown()
{
    ...
    if (bd->InstalledCallbacks)
        ImGui_ImplGlfw_RestoreCallbacks(bd->WindowWrapper);
#ifdef EMSCRIPTEN_USE_EMBEDDED_GLFW3
...
static void ImGui_ImplGlfw_UpdateMouseData()
{
    ...
    {
        GLFWwindow* window = bd->WindowWrapper;
#ifdef EMSCRIPTEN_USE_EMBEDDED_GLFW3
...
static void ImGui_ImplGlfw_UpdateMouseCursor()
{
    ImGuiIO& io = ImGui::GetIO();
    ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
    if ((io.ConfigFlags & ImGuiConfigFlags_NoMouseCursorChange) || glfwGetInputMode(bd->WindowWrapper, GLFW_CURSOR) == GLFW_CURSOR_DISABLED)
        return;
...
void ImGui_ImplGlfw_NewFrame()
{
    ...
    glfwGetWindowSize(bd->WindowWrapper, &w, &h);
    glfwGetFramebufferSize(bd->WindowWrapper, &display_w, &display_h);
...
static EM_BOOL ImGui_ImplGlfw_OnCanvasSizeChange(int event_type, const EmscriptenUiEvent* event, void* user_data)
{
    ...
    glfwSetWindowSize(bd->WindowWrapper, (int)canvas_width, (int)canvas_height);
    return true;
}
