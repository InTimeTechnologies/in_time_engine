...
struct ImGuiPopupData
{
    ...
    ImGuiWindow*        WindowWrapper;         // Resolved on BeginPopup() - may stay unresolved if user never calls OpenPopup()
...
{
    ImGuiWindow*        WindowWrapper;         // Init,Move    // Best candidate window (result->ItemWindow->RootWindowForNav == request->Window)
    ...
    void Clear()        { WindowWrapper = NULL; ID = FocusScopeId = 0; ItemFlags = 0; SelectionUserData = -1; DistBox = DistCenter = DistAxial = FLT_MAX; }
};
...
struct IMGUI_API ImGuiMultiSelectState
{
    ...
    ImGuiMultiSelectState() { WindowWrapper = NULL; ID = 0; LastFrameActive = LastSelectionSize = 0; RangeSelected = NavIdSelected = -1; RangeSrcItem = NavIdItem = ImGuiSelectionUserData_Invalid; }
};
