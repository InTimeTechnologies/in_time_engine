...
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(bs->WindowWrapper == window);
...
ImGuiMultiSelectIO* ImGui::EndMultiSelect()
{
    ...
    IM_ASSERT(g.CurrentMultiSelect != NULL && storage->WindowWrapper == g.CurrentWindow);
...
void ImGui::DebugNodeMultiSelectState(ImGuiMultiSelectState* storage)
{
    ...
    bool open = TreeNode((void*)(intptr_t)storage->ID, "MultiSelect 0x%08X in '%s'%s", storage->ID, storage->WindowWrapper ? storage->WindowWrapper->Name : "N/A", is_active ? "" : " *Inactive*");
...
static bool IsRootOfOpenMenuSet()
{
    ...
    if (window->DC.NavLayerCurrent != upper_popup->ParentNavLayer)
        return false;
    return upper_popup->WindowWrapper && (upper_popup->WindowWrapper->Flags & ImGuiWindowFlags_ChildMenu) && ImGui::IsWindowChildOf(upper_popup->WindowWrapper, window, true);
}
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        ImGuiWindow* child_menu_window = (child_popup && child_popup->WindowWrapper && child_popup->WindowWrapper->ParentWindow == window) ? child_popup->WindowWrapper : NULL;
