...
namespace gl {
	...
	bool gl::MultiLine::s_init(const std::filesystem::path& parentPathToShaders) {
		...
		gl::Shader vertexShader = gl::Shader(gl::Shader::Type::VERTEX);
		gl::Shader fragmentShader = gl::Shader(gl::Shader::Type::FRAGMENT);
	...
	void gl::MultiLine::s_render(gl::Buffer* transformBuffer, glm::mat4* viewProjectionMatrix) {
		...
		for (MultiLine* multiLine : s_multiLineList) {
			...
			if (multiLine->buffer.getMemorySize() == sizeof(GPUData) * multiLine->gpuData.size())
				multiLine->buffer.subwrite(0, sizeof(GPUData) * multiLine->gpuData.size(), multiLine->gpuData.data());
			else
				multiLine->buffer.allocate(sizeof(GPUData) * multiLine->gpuData.size()), multiLine->gpuData.data();
